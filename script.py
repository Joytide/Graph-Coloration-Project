	...
	def triple_coloration(self):
		...
		while len(uncolored)!=0:
			...
			if len(node_neighbors_color)>=3:
				...
				self.setnode_color(node,[x for x in self.three_color if x not in node_neighbors_color][0])
				print("Colored",node,"in",[x for x in self.three_color if x not in node_neighbors_color][0])
